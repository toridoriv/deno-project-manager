...
import { getCurrentVersion, SemVer, VersionObject } from "@tools/versioning.ts";
...
describe("function getCurrentVersion", () => {
  ...
  it("should create a version object based on the given version", () => {
    expect(version.version).to.equal("1.0.0");
    expect(version.tag).to.equal("v1.0.0");
  });
});
